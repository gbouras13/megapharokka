...
INPHARED_NAMES = ["1Aug2023_genomes.fa.msh", "1Aug2023_data.tsv"]
...
def check_envhogs_installation_annot(db_dir):
    # ENVHOG files
...
def check_envhogs_installation_hhsuite(db_dir):
    # ENVHOG files
    for file_name in ENVHOG_NAMES_HHSUITE:
        path = os.path.join(db_dir, file_name)
        if os.path.isfile(path) == False:
...
def check_envhogs_installation_pyhmmer(db_dir):
    # ENVHOG files
    for file_name in ENVHOG_NAMES_PYHMMER:
        path = os.path.join(db_dir, file_name)
        if os.path.isfile(path) == False:
...
def check_db_installation(db_dir):
    check_envhogs_installation_annot(db_dir)
    check_envhogs_installation_hhsuite(db_dir)
    check_envhogs_installation_pyhmmer(db_dir)
    check_envhogs_installation_mmseqs2(db_dir)
