...
def check_envhogs_installation(db_dir):
    # ENVHOG files
    for file_name in ENVHOG_NAMES:
        path = os.path.join(db_dir, file_name)
        if os.path.isfile(path) == False:
            logger.warning(f"ENVHOGs Database file {path} is missing.")
            logger.warning("Please copy them in after this is downloaded :)")
